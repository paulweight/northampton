...
<!-- ########################## -->
		...
		<p class="first">Browse events below.</p>
...
?>
		...
		<div class="cate_info">
			<h2><a href="http://<?php print $DOMAIN; ?>/site/scripts/events.php?categoryID=<?php print $rootCat->id;?>"><?php print $rootCat->name; ?></a></h2>
<?php
			if (sizeof($splitArray['left']) > 0) {
				print '<ul class="info_left list">';
				foreach ($splitArray['left'] as $cat) {
?>
				<li><a href="http://<?php print $DOMAIN; ?>/site/scripts/events.php?categoryID=<?php print $cat->id;?>"><?php print $cat->name; ?></a></li>
<?php
				}
				print '</ul>';
			}
